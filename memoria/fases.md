...
- [x] **F5 — `ACTION_STOP` cancela notificación FSI**: implementado en Sesion 37 junto con parada de audio, wake lock y restauracion de volumen.
- [x] **F6 — `FLAG_DISMISS_KEYGUARD` + `requestDismissKeyguard()`**: implementado en `AlarmActivity` en Sesion 37. Pendiente validar en Redmi bloqueado.
- [x] **F7 — Guía al permiso MIUI "ventanas emergentes"**: fila especifica para Xiaomi/Redmi/Poco en Ajustes, con intent al editor MIUI y fallback a detalles de app. Pendiente validar el path exacto en el Redmi.
